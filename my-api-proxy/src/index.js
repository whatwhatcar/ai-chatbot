		...
		if (request.method === "POST" && url.pathname === "/api/chat") {
			if (!env.OPENAI_API_KEY) {
				return jsonResponse(
					{
						error:
							"Missing OPENAI_API_KEY. For local dev: create my-api-proxy/.dev.vars with OPENAI_API_KEY=sk-... and restart wrangler. For production: npx wrangler secret put OPENAI_API_KEY then deploy.",
					},
					503,
					corsHeaders
				);
